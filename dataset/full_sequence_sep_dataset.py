...
from transforms.keypoints_transform import normalize, transform_to_svd_components
...
class FullSequenceSepDataset(BaseMouseDataset):
    ...
    def reshape_nested(self, N, data):
        if isinstance(data, Mapping):
            ...
            return data.view(N, self.max_seq_length, -1)
        return data
    ...
    def get_sample(self, sequence: np.ndarray) -> dict:
        ...
        attention_mask = torch.ones(N, self.max_seq_length, NUM_MICE, 1, dtype=torch.bool)

        feats = feats.reshape(N, self.max_seq_length, NUM_MICE, -1)
        ...
        segments = self.create_segments(N, feats.shape[1], self.max_seq_length)
